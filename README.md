...
* Open the node_modules/react-native-ble-manager/ios folder and drag BleManager.xcodeproj into your
  Libraries group.
* Check the "Build Phases"of your project and add "libBleManager.a" in the "Link Binary With
  Libraries" section.
...
* Android API >= 23 require the ACCESS_COARSE_LOCATION permission to scan for peripherals. React
  Native >= 0.33 natively support PermissionsAndroid like in the example.
* Before write, read or start notification you need to call `retrieveServices` method

## Example

Look in the [example](https://github.com/innoveit/react-native-ble-manager/tree/master/example)
project.
...
* `showAlert` - `Boolean` - [iOS only] Show or hide the alert if the bluetooth is turned off during
  initialization
* `restoreIdentifierKey` - `String` - [iOS only] Unique key to use for CoreBluetooth state
  restoration
...
Attempts to connect to a peripheral. In many case if you can't connect you have to scan for the
peripheral before. Returns a `Promise` object.
...
Start the notification on the specified characteristic, you need to call `retrieveServices` method
before. Returns a `Promise` object.
...
Enable a service to receive message from BLE pheripherals, a new message trigger a
BleManagerDidReceivedData event. Returns a `Promise` object.
...
Read the current value of the specified characteristic, you need to call `retrieveServices` method
before. Returns a `Promise` object.
...
Write with response to the specified characteristic, you need to call `retrieveServices` method
before. Returns a `Promise` object.
...
`convert-string` or other npm package in order to achieve that. Install the package first:
...
Feel free to use other packages or google how to convert into byte array if your data has other
format.
...
Write without response to the specified characteristic, you need to call `retrieveServices` method
before. Returns a `Promise` object.
...
* `queueSleepTime` - `Integer` - (Optional) specify the wait time before each write if the data is
...
Start the bonding (pairing) process with the remote device. Returns a `Promise` object. The promise
is resolved when either `new bond successfully created` or `bond already existed`, otherwise it will
be rejected.
...
Removes a disconnected peripheral from the cached list. It is useful if the device is turned off,
because it will be re-discovered upon turning on again. Returns a `Promise` object.
...
Check whether a specific peripheral is connected and return `true` or `false`. Returns a `Promise`
object.
...
* `name` - `String` - the name of the peripheral <<<<<<< HEAD
...
* `advertising` - `JSON` - the advertising payload, according to platforms: - [Android] contains the
  raw `bytes` and `data` (Base64 encoded string) - [iOS] contains a JSON object with different keys
  according to
  [Apple's doc](https://developer.apple.com/documentation/corebluetooth/cbcentralmanagerdelegate/advertisement_data_retrieval_keys?language=objc),
  here are some examples: - `kCBAdvDataChannel` - `Number` - `kCBAdvDataIsConnectable` - `Number` -
  `kCBAdvDataLocalName` - `String` - `kCBAdvDataManufacturerData` - `JSON` - contains the raw
  `bytes` and `data` (Base64 encoded string)
  > > > > > > > fdfaef0ce834816b49ce076bce9063ff5ad43851
